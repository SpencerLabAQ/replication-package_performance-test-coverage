...
# %%
#Used in the Paper
#Plotting JMH vs. JUnit Coverage Graph

# Load the data
coverage_jmh = pd.read_csv('../../outputs/coverageJMH.csv')
coverage_jmh = coverage_jmh.rename(columns={'Coverage Percentage': 'JMH Benchmarks'})
coverage_junit = pd.read_csv('../../outputs/coverageJUnit.csv')
coverage_junit = coverage_junit.rename(columns={'Coverage Percentage': 'JUnit Tests'})
...
plt.tight_layout()
plt.savefig(os.path.join(plots_path,'plt_coverage_compare_junit_jmh.pdf'), format='pdf',bbox_inches='tight')
